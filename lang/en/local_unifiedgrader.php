...
$string['bbb_didnotattend_sessions_desc'] = 'This activity has recorded sessions, but attendance data shows this student was not in any of them. You can still enter a grade and feedback.';
...
$string['bbb_filter_noattribution'] = 'Showing every session: this student\'s attendance was recorded without saying which session it belongs to, so the sessions they were in cannot be identified.';
$string['bbb_filter_unreconciled'] = 'Showing every session: this student\'s attendance does not correspond to any of the recordings below, so the sessions they were in cannot be identified.';
$string['bbb_filter_norosters'] = 'Showing every session: no attendance has been recorded against these recordings yet, so sessions this student missed cannot be told apart from ones they attended. Use "Refresh attendance" above.';
$string['bbb_filter_iddiagnostic'] = 'Attendance refers to session <code>{$a->sessionref}</code>, while the first recording listed is <code>{$a->recordingref}</code>. These must match for the session list to be filtered.';
$string['bbb_engagement_refresh_button'] = 'Refresh attendance';
$string['bbb_engagement_refresh_help'] = 'Re-read each recording\'s attendance from the BigBlueButton server. This is also what records who was in which session, which the session list above is filtered by — run it once per activity after upgrading, and again if a recording is added.';
...
$string['bbb_engagement_scrape_result'] = 'Done — read {parsed} of {recordings} recording(s); matched {matched}, unmatched {unmatched}. Recordings that could not be read keep no attendance, so every student still sees them.';
